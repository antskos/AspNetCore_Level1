Cart = {
    ...
    initEvents: function () {
        $(".add-to-cart").click(Cart.addToCart);        // обработчик события "click" в классе "add-to-cart"
    ...
    addToCart: function (event) {
        event.preventDefault(); /*Отменяем стандартное поведение браузера по нажатию ссылки*/

        var button = $(this);
        const id = button.data("id"); // data-id="..."

        $.get(Cart._properties.addToCartLink + "/" + id)        // отправка запроса на сервер
            .done(function () {                                 // удачное завершение запроса: вызывает функции ниже
                Cart.showToolTip(button);
                Cart.refreshCartView();
            })
            .fail(function () { console.log("addToCart fail"); });      // неудачное завершение запроса
    },
    ...
    refreshCartView: function () {
        var container = $("#cart-container");       // нахождение контейнера,который отображает корзину
        $.get(Cart._properties.getCartViewLink)     // запрос Get на сервер корзины
